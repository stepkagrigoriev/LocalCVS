...
class TagTest(unittest.TestCase):
    def setUp(self):
        ...
        with open('.cvs/refs/heads/master', 'w') as f:
            ...
            f.write('sha_to_some_commit2')
            # будто уже есть коммит, у которого мы точно знаем sha
        with open('.cvs/HEAD', 'w') as f:
            f.write('refs/heads/master')
            # и HEAD как раз указывает на этот коммит
    ...
    def test_create_tag_to_sha(self):
        with contextlib.redirect_stdout(io.StringIO()):
            run_command('tag', ['TAG', 'sha_to_some_commit1'])
    ...
    def test_create_tag_to_head(self):
        ...
        with open(tag_path, 'r') as f:
            self.assertEqual(f.read().strip(),
                             'sha_to_some_commit1sha_to_some_commit2')
    ...
    def test_get_tag_commit(self):
        with contextlib.redirect_stdout(io.StringIO()):
            run_command('tag', ['AGAKANESHNO', 'sha_to_some_commit1'])
    ...
    def test_list_tags(self):
        with contextlib.redirect_stdout(io.StringIO()):
            run_command('tag', ['tag1', 'sha_to_some_commit1'])
            run_command('tag', ['tag2', 'sha_to_some_commit2'])
